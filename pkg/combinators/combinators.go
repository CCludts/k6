// Package combinators implements a simple parser combinator library.
package combinators
...
func Char(c rune) Parser {
	return func(input []rune) Result {
		if len(input) == 0 || input[0] != c {
			return Failure(NewError(input, string(c)), input)
		}
...
func Digit() Parser {
	return func(input []rune) Result {
		...
		// of UTF-8 characters, we need to substract the actual rune
...
func Tag(t string) Parser {
	termRunes := []rune(t)

	return func(input []rune) Result {
		if len(input) < len(termRunes) {
			return Failure(NewError(input, t), input)
		}
		...
		return Success(t, input[len(termRunes):])
	}
}
...
func Float() Parser {
	...
	return func(input []rune) Result {
		...
		if err != nil {
			err = fmt.Errorf("failed to parse '%v' as float; reason: %v", parsed, err)
